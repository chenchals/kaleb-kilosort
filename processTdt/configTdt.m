clear ops
ops.fs                  = 24414; %Sampling rate of TDT
ops.GPU                 = useGPU; %useGPU; % whether to run this code on an Nvidia GPU (much faster, mexGPUall first)		
...
ops.percentSamplesToUse = percentSamplesToUse ; % of data per channel to use. useful for troubleshooting purposes
ops.nSpikesPerBatch     = 10000; % emouse has 4000, is this spike rate?
...
ops.Nchan               = 32;
ops.chOffset            = 32;
ops.chanMap = [pathToYourConfigFile 'tdtChanMap'];%1:ops.Nchan; % treated as linear probe if unavailable chanMap file		

ops.Nfilt               = 64;  % number of clusters to use (2-4 times more than Nchan, should be a multiple of 32)     		
ops.nNeighPC            = 12; %12;%12; % visualization only (Phy): number of channnels to mask the PCs, leave empty to skip (12)		
ops.nNeigh              = 16; % visualization only (Phy): number of neighboring templates to retain projections of (16)		
		
% options for channel whitening		
ops.whitening           = 'full';% 'full'; % type of whitening (default 'full', for 'noSpikes' set options for spike detection below)		
...
ops.ForceMaxRAMforDat   = 40e9; % maximum RAM the algorithm will try to use; on Windows it will autodetect.
